...
namespace Tests
{
	...
	{
		[Test]
		public static void ParseUrlIsCorrect()
		{
		...
		[Test]
		public static void ParseUrlIsNOTCorrect()
		{
		...
		public void LogicUnitTest()
		{
			...
			ApiTestUnit.Setup(api => api.ResolvePageAddress("group")).Returns(
				new ResolveResult()
				{
					Id = 10,
			...
			ApiTestUnit.Setup(api => api.GetWallPosts(2, 10, 0)).Returns(
				new List<Post>()
				{
					...
					new Post()
					{
						...
						Date = 3 * 24 * 3600			// этот пост не попадает в выборку за последнюю неделю
					}
				});
